...
namespace MetroUI
{
  public partial class harcamalarForm : MetroForm
  {
      private int _ID, islem;

    public harcamalarForm(int pr1,int islem)
    {
        if (pr1 == 0 && islem == 0)
        {
    ...
    private void harcamalarForm_Load(object sender, EventArgs e)
    {
        if (islem == 0)
        {
            ...
            if (drEd.Read())
            {
                textBox3.Text = drEd["kasa_alinan"].ToString();
